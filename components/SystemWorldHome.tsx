...
const COPY = {
  de: {
    ...
    thesis: 'Ein System für Arbeit mit KI – nicht nur ein Chatfenster.',
    intro:
      'Saimôr ist der Rahmen. Das OS ist der digitale Arbeitsraum. Môra hält den Kontext zusammen. Der Desk ist die Oberfläche, auf der du damit arbeitest.',
    enter: 'Security Check starten',
    login: 'Einloggen',
    learn: 'System verstehen',
    training: 'Vorträge & Schulungen',
    entryNote: 'Der Security Check wird zum ersten echten Objekt in deinem 30-Tage-Demoraum.',
    orbit: [
      ['OS', 'Arbeitsraum'],
      ['MÔRA', 'Kontext'],
      ['DESK', 'Oberfläche'],
    ...
    systemTitle: 'Vier Ebenen. Eine gemeinsame Welt.',
    systemIntro:
      'Nicht vier lose Produkte nebeneinander. Jede Ebene hat eine klare Aufgabe und greift auf denselben Zusammenhang zurück.',
    layers: [
      ['00', 'SAIMÔR', 'System / Studio', 'Die Klammer über allem: Produktwelt, Architektur und die Idee, KI als Teil eines Arbeitsraums zu bauen.'],
      ['01', 'OS', 'Arbeitsraum', 'Dateien, Vorgänge, Bereiche und Anbindungen leben in einem gemeinsamen digitalen Raum.'],
      ['02', 'MÔRA', 'Kontextschicht', 'Môra hält Beziehungen und Verlauf verfügbar, damit KI nicht bei jeder Aufgabe wieder bei null anfängt.'],
      ['03', 'DESK', 'Oberfläche', 'Die tägliche Arbeitsansicht des Systems – ruhiger, direkter und auf das konzentriert, was gerade zählt.'],
    ],
    entryLabel: 'ENTRY / SECURITY CHECK',
    entryTitle: 'Der Einstieg beginnt mit etwas Echtem.',
    entryText:
      'Du startest mit deiner Domain. Der Report wird zum ersten Objekt in deinem Saimôr-Raum, bleibt mit deinem Zugang verbunden und gibt dem Demo-Raum direkt echten Kontext.',
    entryCta: 'Security Check starten',
    studioEyebrow: 'SAIMÔR · STUDIO',
    studioTitle: 'Produkte bauen. Wissen weitergeben.',
    studioText:
      'Hinter Saimôr steht Marius Fahrländer – aus IT-Systemmanagement und technischem Vertrieb. Neben der Produktarbeit entstehen Vorträge, Workshops und Schulungen rund um praktische KI-Anwendungen.',
    offers: [
  ...
  en: {
    eyebrow: 'SAIMÔR · SOVEREIGN SYSTEMS',
    thesis: 'A system for working with AI – not just another chat window.',
    intro:
      'Saimôr is the frame. The OS is the digital workspace. Môra keeps the context together. The Desk is the surface you work on.',
    enter: 'Start Security Check',
    login: 'Log in',
    learn: 'Understand the system',
    training: 'Talks & training',
    entryNote: 'The Security Check becomes the first real object inside your 30-day demo space.',
    orbit: [
      ['OS', 'Workspace'],
      ['MÔRA', 'Context'],
      ['DESK', 'Surface'],
    ...
    systemTitle: 'Four layers. One shared world.',
    systemIntro:
      'Not four disconnected products. Each layer has one clear job and works with the same surrounding context.',
    layers: [
      ['00', 'SAIMÔR', 'System / Studio', 'The frame around everything: product world, architecture and the idea of building AI as part of a workspace.'],
      ['01', 'OS', 'Workspace', 'Files, cases, areas and connections live inside one shared digital space.'],
      ['02', 'MÔRA', 'Context layer', 'Môra keeps relationships and history available so AI does not restart from zero every time.'],
      ['03', 'DESK', 'Surface', 'The daily working view of the system – calmer, more direct and focused on what matters now.'],
    ],
    entryLabel: 'ENTRY / SECURITY CHECK',
    entryTitle: 'Entry starts with something real.',
    entryText:
      'You start with your domain. The report becomes the first object inside your Saimôr space, stays connected to your access and gives the demo space real context immediately.',
    entryCta: 'Start Security Check',
    studioEyebrow: 'SAIMÔR · STUDIO',
    studioTitle: 'Build products. Share what works.',
    studioText:
      'Behind Saimôr is Marius Fahrländer, with a background in IT systems management and technical sales. Alongside the products, he gives talks, workshops and training around practical uses of AI.',
    offers: [
  ...
  return (
    <div className="overflow-hidden bg-[#06100d] text-white selection:bg-[#d8b86a] selection:text-black">
      <section className="relative min-h-[100svh] overflow-hidden border-b border-white/[0.08] bg-[#07110e]">
        <div
          className="absolute inset-0 bg-cover bg-[center_44%] opacity-[0.34] saturate-[0.62] contrast-[1.08] sm:bg-center"
          style={{ backgroundImage: `url('${FOREST_URL}')` }}
          aria-hidden="true"
        />
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(3,10,8,.88)_0%,rgba(3,10,8,.72)_43%,rgba(3,10,8,.48)_70%,rgba(3,10,8,.70)_100%)]" />
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(2,7,6,.24)_0%,rgba(3,10,8,.44)_58%,#06100d_100%)]" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_72%_36%,rgba(214,168,72,.13),transparent_27%),radial-gradient(circle_at_68%_52%,rgba(89,205,177,.09),transparent_35%),radial-gradient(circle_at_18%_38%,rgba(89,205,177,.055),transparent_30%)]" />
        <div className="s-fog-a pointer-events-none absolute -left-[15%] top-[10%] h-[70%] w-[70%] rounded-full bg-[#7fd4c1]/[0.045] blur-[140px]" />
        <div className="s-fog-b pointer-events-none absolute -right-[18%] top-[12%] h-[72%] w-[72%] rounded-full bg-[#d6a848]/[0.055] blur-[160px]" />
        <div className="absolute inset-0 opacity-[0.11] [background-image:linear-gradient(rgba(255,255,255,.028)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.028)_1px,transparent_1px)] [background-size:52px_52px] [mask-image:linear-gradient(to_bottom,black,transparent_88%)]" />
        ...
        <div className="relative mx-auto flex min-h-[100svh] max-w-7xl flex-col px-5 pb-5 pt-[84px] sm:px-8 sm:pb-8 sm:pt-28 lg:px-10">
          <div className="flex items-center justify-between gap-4">
            <div className="font-mono text-[8px] font-semibold tracking-[0.24em] text-white/40 sm:text-[10px] sm:tracking-[0.28em]">{c.eyebrow}</div>
            <div className="flex items-center gap-2 font-mono text-[8px] tracking-[0.16em] text-white/32 sm:text-[10px]">
          ...
          <div className="grid flex-1 items-center gap-3 py-4 sm:gap-8 sm:py-8 lg:grid-cols-[.92fr_1.08fr] lg:gap-8 lg:py-0">
            <div className="order-2 lg:order-1 lg:pr-8">
              <div className="max-w-2xl">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 text-[11px] text-white/40 sm:text-xs">
                  <Link href="/login?callbackUrl=%2Faccount%2Fbridge" className="inline-flex items-center gap-1.5 transition hover:text-white/72">
                    <LogIn className="h-3.5 w-3.5" />{c.login}
                  </Link>
                  <span className="hidden h-3 w-px bg-white/14 sm:block" />
                  <a href="#studio" className="inline-flex items-center gap-1.5 text-[#e7ca84]/64 transition hover:text-[#f0d998]">
                    <Mic2 className="h-3.5 w-3.5" />{c.training}
                  </a>
                </div>
                <p className="mt-4 max-w-lg text-[11px] leading-5 text-white/32 sm:text-xs">{c.entryNote}</p>
              </div>
            </div>
      ...
      <section id="system" className="relative border-b border-white/[0.08] bg-[radial-gradient(circle_at_80%_10%,rgba(127,212,193,.045),transparent_32%),#06100d] px-5 py-20 sm:px-8 md:py-28 lg:px-10">
        <div className="mx-auto max-w-7xl">
          ...
          <p className="mt-6 max-w-2xl text-sm leading-7 text-white/42 sm:text-base">{c.systemIntro}</p>
