...
export default function MoraDashboardConnection() {
  ...
  const [dashboardNodeEngaged, setDashboardNodeEngaged] = useState(false);
  ...
  useEffect(() => {
    ...
    const handleNodeHover = (event: Event) => {
      const detail = Boolean((event as CustomEvent<boolean>).detail);
      setDashboardNodeEngaged(detail);
    };
    window.addEventListener('mora-dashboard-card-hover', handleNodeHover as EventListener);
    return () => window.removeEventListener('mora-dashboard-card-hover', handleNodeHover as EventListener);
  }, []);
  ...
  const connectionReady = dashboardInView && orbHovering && dashboardNodeEngaged;
  ...
  const shouldRenderPath = showConnection && !prefersReducedMotion;
  const shouldRenderGlow = showConnection && prefersReducedMotion;
        ...
        {shouldRenderGlow && (
          ...
          >
            <motion.div
              ...
              style={{
                background: 'radial-gradient(circle, rgba(212, 180, 131, 0.35) 0%, transparent 70%)',
                filter: 'blur(10px)'
              }}
