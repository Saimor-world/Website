...
export const metadata = {
  ...
  description: 'Saimôr’s first real XRPL product and payment loop.',
};
    ...
    <main className="min-h-screen bg-[#050908] px-5 py-8 text-white md:px-10 md:py-12">
      <div className="mx-auto max-w-5xl">
        <header className="mb-10 flex items-center justify-between gap-4">
          <Link href="/en" className="inline-flex items-center gap-2 text-xs text-white/38 transition hover:text-white/72">
        ...
        <div className="mb-8 max-w-3xl">
          ...
          <p className="mt-6 max-w-2xl text-base font-light leading-relaxed text-white/42 md:text-lg">
            Not a shop system. Not finance theatre. One small real purchase proving Saimôr can understand money as an event.
          </p>
        </div>
        ...
        <div className="mt-6 grid gap-3 text-[10px] text-white/28 md:grid-cols-3">
          <div className="rounded-2xl border border-white/[0.05] bg-white/[0.015] p-4">The receiving address exists only in the server environment.</div>
          <div className="rounded-2xl border border-white/[0.05] bg-white/[0.015] p-4">Xaman creates the signature. Saimôr never receives a seed or private key.</div>
          <div className="rounded-2xl border border-white/[0.05] bg-white/[0.015] p-4">Unlock happens only after validated on-ledger verification.</div>
        </div>
      </div>
    </main>
  );
